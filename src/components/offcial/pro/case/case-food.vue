<template>
  <div class="caseFood">
    <div>
      <div class="banner">
        <p>餐饮服务</p>
        <p>不管生活怎么忙碌，美食是不可或缺的</p>
      </div>
      <div class="cont">
        <div class="list">
          <ul>
            <li v-for="(item, index) in proList" :key="index">
              <div class="listItem" @click="checkIn(item.industry)">
...
export default {
  ...
  methods: {
    checkIn (e) {
      ...
      // let num = e
      let that = this
      that.$axios.post('', qs.stringify({})).then(function () {
        console.log('点击键入')
      })
    }
  }
...
.caseFood .banner{
  ...
  background-image: url('./../../../../assets/img/case/caseFood/banner.png');
  background-size: cover;
  text-align: left;
  padding: 9.125rem 0  9.25rem 23rem;
}
.caseFood .banner p:first-child{
  font-size: 3rem;
  color: #ffffff;
  margin-bottom: 2rem;
}
.caseFood .banner p:last-child{
  font-size: 1rem;
  color: #ffffff;
}
.caseFood .cont{
...
.caseFood .cont .list ul li:last-child{
  margin-right: 0;
}
.caseFood .cont .listItem{
  position: relative;
}
.caseFood .cont .list .listBg{
  margin: 0 auto;
  width: 17.5rem;
  height: 18.3rem;
}
.caseFood .cont .list .listDetail{
  width: 16.76rem;
  height: 9.125rem;
  margin: 0 auto;
...
.caseFood .cont .list .listItem:hover .listDetail{
  display: block;
}
.caseFood .cont .list .listDContText{
  font-size: 1rem;
  color: #333333;
  box-sizing: border-box;
